...
function initBooking() {
  ...
  bookingForm.addEventListener('submit', async function(e) {
    e.preventDefault();
    const name = nameInput?.value.trim(); const phone = phoneInput?.value.trim(); const guests = guestsInput?.value || '2'; const comment = commentInput?.value.trim() || ''; const date = dateInput?.value; const time = timeInput?.value;
    ...
    try {
      ...
      bookingModal.style.display = 'none'; bookingModal.classList.remove('modal--open'); document.body.style.overflow = "";
      bookingForm.reset();
    ...
    } catch (error) { console.error(error); showToast('❌ Ошибка при отправке'); }
