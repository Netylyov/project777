...
/* ===========================
   ПРИНУДИТЕЛЬНЫЕ ИСПРАВЛЕНИЯ ИНТЕРФЕЙСА
=========================== */
function fixInterface() {
  console.log('🔧 Запуск принудительных исправлений интерфейса...');
  
  // 1. ПЕРЕМЕЩАЕМ КРЕСТИК В ПРАВЫЙ УГОЛ
  ...
  // 2. ДОБАВЛЯЕМ РАМКИ ВСЕМ КНОПКАМ
  const allButtons = document.querySelectorAll('button');
  allButtons.forEach(btn => {
    // Пропускаем кнопки, которые не должны иметь рамок
    const skipButtons = ['burgerBtn', 'cart-close', 'history-close', 'closeBooking'];
    if (skipButtons.includes(btn.id)) return;
    
    // Добавляем стили принудительно
    btn.style.transition = 'all 0.3s ease';
    
    // Определяем тип кнопки по классам или ID
    ...
    else if (btn.classList.contains('burger-btn')) {
      // Оставляем как есть
    }
    else {
      // Для остальных кнопок добавляем стандартную рамку
      btn.style.background = 'transparent';
      btn.style.color = '#fff';
      btn.style.border = '1px solid rgba(255,255,255,0.3)';
      btn.style.borderRadius = '8px';
  ...
  console.log('✅ Все кнопки получили рамки');
  
  // 3. ДОБАВЛЯЕМ СТИЛИ ДЛЯ КНОПОК В МОДАЛКЕ КОРЗИНЫ
  ...
  // 4. ДОБАВЛЯЕМ СТИЛИ ДЛЯ КНОПКИ ОЧИСТКИ ИСТОРИИ
  ...
  if (historyHeader) {
    ...
    historyHeader.style.paddingBottom = '15px';
    historyHeader.style.borderBottom = '1px solid rgba(255,255,255,0.1)';
  }
  
  console.log('✅ Принудительные исправления завершены');
}

// Запускаем исправления после загрузки страницы
setTimeout(() => {
  fixInterface();
}, 500);